...
class SOM(object):
    ...
    def _batchtrain(self, trainlen, radiusin, radiusfin, njob=1,
        ...
        for i in range(trainlen):
            ...
            self.codebook.matrix = self.update_codebook_voronoi(data, bmu,
                                                                neighborhood)
            #location, terror = self.calculate_topographic_error()
            terror = self.calculate_topographic_error()
            #print location
            # self.location_matrix.append(location)
            qerror = (i + 1, round(time() - t1, 3),
                      np.mean(np.sqrt(bmu[1] + fixed_euclidean_x2)), terror)
            
            logging.info(
                " epoch: %d ---> elapsed time:  %f, quantization error: %f, topographic error: %f" %
                qerror)
            # print 'quantization error:', qerror, 'topographic error:\n' , terror
    ...
    def get_bmulocation(self):
        bmus1 = self.find_bmu(self.data_raw, njb=1, nth=1)
        return self.bmu_ind_to_xy(np.array(bmus1[0]))[:, 0:2][0]
    ...
    def calculate_topographic_error(self):
        ...
        # print self.bmu_ind_to_xy(np.array(bmus1[0]))[:,0:2]
        # return self.bmu_ind_to_xy(np.array(bmus1[0]))[:, 0:2][0],np.mean(bmus_gap != 1)
